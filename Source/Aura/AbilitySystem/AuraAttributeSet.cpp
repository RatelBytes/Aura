...
void UAuraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	// In case attribte that is being changed is IncomingDamage. We use this attribute to store the damage and then doing something with it. Applying armor, resistances etc.
	if (Data.EvaluatedData.Attribute == GetIncomingDamageAttribute())
	{
		// we "consume" the incoming damage and store it in a local variable to work with. Then we reset IncomingDamage.
		...
		if (LocalIncomingDamage > 0.f)
		{
			...
			{
				// We try to activate HitReact GameplayAbility by asking to activate an ability corresponding to specific Tag.
				// For that to work, we need to make sure, that GA_HitReact has TAG assigned to it (Effects.HitReact)
...
{
	// Check if it is not self-damage. Then show Damage
...
{
	// Macro that notifies GAS that Health has changed. Needed for GAS core functionality
